...
class RecordApiDelegateImplTest {
    ...
    void updateRecord() throws Exception {
        try (MockedStatic<CurrentUserUtility> mock = Mockito.mockStatic(CurrentUserUtility.class)) {
            ...
            Mockito.when(recordService.getRecordById(record.getId()))
                    .thenReturn(Optional.of(record));

            Mockito.when(recordService.updateRecord(updateRequest, record)).thenReturn(record);
    ...
    @Test
    void updateRecordUnAuthorize() throws Exception {

        RecordUpdateRequest updateRequest = new RecordUpdateRequest();
        ObjectMapper objectMapper = new ObjectMapper();
        final String postBody = objectMapper.writeValueAsString(updateRequest);

        when(authorizationHandler.isAllowed("update", Record.class)).thenReturn(false);

        // Act and Assert
        mockMvc.perform(
                        put("/api/v1/records/" + UUID.randomUUID())
                                .content(postBody)
                                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isForbidden());
    }
    ...
    @Test
    void updateRecordNullUserType() throws Exception {
        Record record =
                createRecord(
                        createRecordDefinition(), createTransaction(createUser()), createUser());
        ...
        Mockito.when(recordService.getRecordById(record.getId())).thenReturn(Optional.of(record));

        Mockito.when(CurrentUserUtility.getCurrentUser()).thenReturn(null);
    ...
    @Test
    void updateRecordNotAgencyUser() throws Exception {
        try (MockedStatic<CurrentUserUtility> mock = Mockito.mockStatic(CurrentUserUtility.class)) {
            mock.when(CurrentUserUtility::getCurrentUser)
                    .thenReturn(
                            Optional.of(
    ...
    @Test
    void updateRecordUnAuthorizeAdminData() throws Exception {
        Record record =
                createRecord(
                        createRecordDefinition(), createTransaction(createUser()), createUser());
        ...
        when(authorizationHandler.isAllowed("update-admin-data", Record.class)).thenReturn(false);
        when(recordService.hasAdminDataChanges(
                        record, updateRequest.getStatus(), updateRequest.getExpires().toString()))
                .thenReturn(true);
