...
public class ConversationFilters extends BaseFilters {
    ...
    private final List<UUID> entityReferenceIds;
    ...
    public ConversationFilters(
            ...
            List<UUID> entityReferenceIds) {
        super(sortBy, sortOrder, pageNumber, pageSize);
        this.referenceId = referenceId;
        this.referenceType = referenceType;
        this.entityReferenceIds = entityReferenceIds;
    }

    public Specification<Conversation> getConversationByFilters() {
        return (root, query, criteriaBuilder) -> {
            if (entityReferenceIds == null || entityReferenceIds.isEmpty()) {
            ...
            addEntityReferencePredicate(predicates, criteriaBuilder, root);
    ...
    private void addEntityReferencePredicate(
            List<Predicate> predicates, CriteriaBuilder criteriaBuilder, Root<Conversation> root) {
        if (entityReferenceIds != null
                && !entityReferenceIds.isEmpty()
                && (StringUtils.isBlank(referenceType) || referenceId == null)) {
            CriteriaBuilder.In<UUID> inClause =
                    criteriaBuilder.in(root.get("entityReference").get("id"));
            for (UUID id : entityReferenceIds) {
                inClause.value(id);
            }
