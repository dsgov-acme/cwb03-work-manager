...
public class RideRequestServiceImpl implements RideRequestService {
    ...
    public Optional<PromiseTimeResponse> getPromiseTimes(PromiseTimeRequest request) {
        ...
        log.info("Successfully retrieved promise times for request: {}", request);
    ...
    public Optional<ReservationDetailsResponse> submitReservation(
        ...
        try {
            ...
            log.error("Failed to submit reservation for request {}", request, e);
            if (NOT_FOUND == e.getStatusCode()) {
                return Optional.empty();
            }
            throw e;
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new ApiException(
                    "Failed to submit reservation  for request.  Status code: "
                            + response.getStatusCode());
        }
    ...
    public Optional<ReservationDetailsResponse> getReservationDetails(
        ...
        try {
            ...
            log.error("Failed to retrieve reservation details for request {}", request, e);
            if (NOT_FOUND == e.getStatusCode()) {
                return Optional.empty();
            }
            throw e;
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new ApiException(
                    "Failed to retrieve reservation details for request.  Status code: "
                            + response.getStatusCode());
        }
    ...
    private void prepareRequest(PromiseTimeRequest request) {
        try {
            ...
            log.info("Promise Time Request drop placeId already converted. Continuing.");
        } catch (Exception e) {
        ...
        try {
            ...
            log.info("Promise Time Request pickup placeId already converted. Continuing.");
        } catch (Exception e) {
            log.error("Unexpected error when preparing request to retrieve promise times", e);
        }
    }
